...
type Options = Parameters<typeof useFetch>[1];
 ...
 */
const useHttp = <T>(url: string, options?: Options) => {
  ...
  const res = useFetch(reqUrl, {
    ...options,
    headers: customHeaders,
  }) as AsyncData<ResOptions<T>, any | null>;

  watchEffect(() => {
    if (res.data.value && res.data.value?.code !== 200) {
      showNotify({
        type: "danger",
